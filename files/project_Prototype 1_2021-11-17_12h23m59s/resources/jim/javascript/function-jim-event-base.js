...
//                $target =$("[id="+$wrapper.prop('id')+"]");
			...
			for(t=0, tLen=style.target.length; t<tLen; t+=1) {
                ...
                if($target) {
                  ...
                  if(jQuery.browser.msie) {
                    if(style["attributes-ie"]) {
                      undoStyle["attributes-ie"] = {};
                      for(property in style["attributes-ie"]) {
                        if(style["attributes-ie"].hasOwnProperty(property)) {
                          var i=property.indexOf('#');
                          if(i!=-1){
                            property=property.substring(0,i);
                          }
                          ...
                          if(bShape){
                            undoStyle["attributes-ie"][property] = $target.shapeStyle(property);
                          }
                          else{
                    	    if(property==="-pie-background" || property==="border-radius" || property==="filter" || property==="-ms-filter" || property==="-pie-poll"){
                              if(jimUtil.exists($target.get(0).currentStyle.getAttribute(property)))
                                undoStyle["attributes-ie"][property] = $target.get(0).currentStyle.getAttribute(property);
                              else {
								if(property==="-pie-background" && jQuery.browser.version == 9)
									undoStyle["attributes-ie"][property] = $target.css("background-color");
								else
									undoStyle["attributes-ie"][property] = "";
							  }
                            } else {
                              undoStyle["attributes-ie"][property] = $target.css(property);
                            }
                          }
                        }
                      }
                    }
                    if(style["expressions-ie"]) {
                      undoStyle["expressions-ie"] = {};
                      for(property in style["expressions-ie"]) {
                        if(style["expressions-ie"].hasOwnProperty(property)) {
                          undoStyle["expressions-ie"][property] = $target.css(property);
                        }
                      }
                    }
                  }
	...
	"getNextStyle" : function(attribute, target, attributes) {
		...
		return getCurrentStyle(attribute, target);
	},
